				...
				if ( $questionModel->getId() == intval( $question_id ) ) {
					...
					switch ( $questionData['type'] ) {
						...
						case 'assessment_answer':
							...
							if ( ( ! empty( $userResponse ) ) && ( isset( $questionData['correct'][ $userResponse ] ) ) ) {
								$correct = true;
								if ( isset( $questionData['points'][ $userResponse ] ) ) {
									$points = $questionData['points'][ $userResponse ];
								} else {
									$points = 1;
								}
							}
			...
			if ( ( isset( $result['e'] ) ) && ( ! empty( $result['e'] ) ) ) {
				if ( ( isset( $result['e']['type'] ) ) && ( ! empty( $result['e']['type'] ) ) ) {
					...
					switch ( $result['e']['type'] ) {
						...
						case 'assessment_answer':
							if ( isset( $extra['r'] ) ) {
								$answer_idx   = absint( $extra['r'] ) + 1;
								$response_str = maybe_serialize( array( (string) $answer_idx ) );
							}
							break;
